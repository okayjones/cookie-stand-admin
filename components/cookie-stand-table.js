...
export default function CookieStandTable({ stands, onDelete }) {
    ...
    return (
        <Table>
                ...
                {stands.map((stand, i) => {
                    return (
                        <tr key={stand.id} className="text-right border border-green-500 odd:bg-green-200 even:bg-green-300">
                            <TH>
                                <div>
                                    <p className="float-left pl-4 pr-8">{stand.location}</p>
            ...
            <tfoot>
                <tr className="font-bold text-right bg-green-400">
                    <TH>Totals</TH>
                    {hours.map((_, i) => {
...
function Table({ children }) {
    return (
        <table className="m-2 mx-auto text-sm border border-green-500">
            {children}
        </table>
    );
}
...
function TD({ children }) {
    return (
        <td className="p-1 pl-6 border border-green-500">{children}</td>
    )
}
